...
class SplitFlow(nn.Module):
    ...
    def forward(self, z, ldj, reverse=False):
        prior = torch.distributions.normal.Normal(loc=torch.tensor(0.0, device=z.device), scale=torch.tensor(1.0, device=z.device))
        if not reverse:
            z, z_split = z.chunk(2, dim=1)
            ldj += prior.log_prob(z_split).sum(dim=[1, 2, 3])
        else:
            z_split = prior.sample(sample_shape=z.shape)
