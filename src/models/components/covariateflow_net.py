...
from models.components.blocks.covflow_blocks import CouplingLayer, GatedConvNet, Dequantization,\
...
from models.components.utils.covflow_utils import create_checkerboard_mask, create_channel_mask, create_quadrent_cycle_mask
...
class CovariateFlow(torch.nn.Module):
    ...
    def __init__(
        ...
        img_channels, img_height, img_width= img_size
...
def create_conditional_flow(img_shape=(2, 3, 32, 32), num_coupling_layers = 6,  train_set = None,):
    ...
    flow_layers += [AffineCouplingSdl()]
    flow_model = CovariateFlow(flows=flow_layers, train_set = train_set)

    return flow_model



def create_unconditional_flow(img_shape=(2, 3, 32, 32), num_coupling_layers = 6,  train_set = None,):
...
def create_unconditional_sdl_flow(img_shape=(2, 3, 32, 32), num_coupling_layers = 6,  train_set = None,):
...
def create_conditional_cycle_test_flow(train_set = None):
